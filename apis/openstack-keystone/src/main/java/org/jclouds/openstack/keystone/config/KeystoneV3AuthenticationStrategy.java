...
public class KeystoneV3AuthenticationStrategy  implements AuthenticationStrategy {

    private final AuthenticationApi api;
    private final Supplier<Credentials> credentialsSupplier;

    @Inject
    KeystoneV3AuthenticationStrategy(AuthenticationApi api, @Provider Supplier<Credentials> credentialsSupplier) {
        this.api = api;
    ...
    public String authenticate() {
        ...
        Token token = api.create(tenantName.orNull(), PasswordCredentials.createWithUsernameAndPassword(usernameOrAccessKey, credentialsSupplier.get().credential));
