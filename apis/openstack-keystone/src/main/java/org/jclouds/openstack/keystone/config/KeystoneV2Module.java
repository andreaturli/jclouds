...
public final class KeystoneV2Module extends PrivateModule {

   private static final String VERSION = KeystoneVersions.V2.toString();
   ...
   protected void configure() {
      ...
      bind(AuthenticationStrategy.class).annotatedWith(Names.named(VERSION)).to(KeystoneV2AuthenticationStrategy.class);
...
//      expose(LocationsSupplier.class); //.annotatedWith(Names.named(version));
   ...
   @Provides
   @Singleton
   protected Map<String, Function<Credentials, Access>> provideAuthenticationMethods(Injector i) {
      return authenticationMethods(i);
   }

   protected Map<String, Function<Credentials, Access>> authenticationMethods(Injector i) {
      ImmutableSet.Builder<Function<Credentials, Access>> fns = ImmutableSet.<Function<Credentials, Access>> builder();
