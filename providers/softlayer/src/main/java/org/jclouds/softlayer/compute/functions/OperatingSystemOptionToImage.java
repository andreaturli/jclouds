...
@Singleton
public class OperatingSystemOptionToImage implements Function<OperatingSystemOption, Image> {
    ...
    */
   public static Function<OperatingSystemOption, String> imageId() {
      return new Function<OperatingSystemOption, String>() {
         @Override
         public String apply(OperatingSystemOption operatingSystemOption) {
            checkNotNull(operatingSystemOption, "operatingSystemOption");
            final String softwareDescriptionId = operatingSystemOption.getSoftwareDescriptionId();
            if( softwareDescriptionId == null ) {
              return operatingSystemOption.getOperatingSystemReferenceCode();
            } else {
               return operatingSystemOption.getOperatingSystemReferenceCode() + "_" +
   ...
   @Override
   public Image apply(OperatingSystemOption operatingSystemOption) {
      checkNotNull(operatingSystemOption, "operatingSystemOption");
      String description = checkNotNull(operatingSystemOption.getDescritpion(),
              "operatingSystemOption.itemPrice.item.description");

      OsFamily osFamily = osFamily().apply(description);
      if (osFamily == OsFamily.UNRECOGNIZED) {
         logger.debug("Cannot determine os family for item: %s", operatingSystemOption);
      }
      Integer bits = osBits().apply(description);
      if (bits == null) {
         logger.debug("Cannot determine os bits for item: %s", operatingSystemOption);
      }
      String osVersion = osVersion().apply(description);
      if (osVersion == null) {
         logger.debug("Cannot determine os version for item: %s", operatingSystemOption);
      }
      ...
      return new ImageBuilder()
              .ids(imageId().apply(operatingSystemOption))
              .description(description)
              .operatingSystem(os)
